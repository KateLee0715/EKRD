...
class KGAT(nn.Module):
    ...
    def __init__(self, args,
        ...
        self.relation_dim = args.latdim

        self.aggregation_type = args.aggregation_type
        self.conv_dim_list = [args.latdim] + eval(args.conv_dim_list)
        self.mess_dropout = eval(args.kgat_mess_dropout)
        ...
        self.trans_M = nn.Parameter(torch.Tensor(self.n_relations+1, self.embed_dim, self.relation_dim))
    ...
    def calc_cf_embeddings(self, getMultOrder=False):
        ...
        if not getMultOrder:
            ...
            all_embed = torch.cat(all_embed[2:], dim=1) 
        return all_embed
    ...
    def getEmbeddings(self, getMultOrder=False):
        all_embed = self.calc_cf_embeddings(getMultOrder) 
        if not getMultOrder:
            return all_embed, self.relation_embed.weight
        else:
            return all_embed
    ...
    def forward(self, *input, mode, getMultOrder=False):
        if mode == 'train_cf':
        ...
        if mode == 'getEmbeds':
            return self.getEmbeddings(getMultOrder)
