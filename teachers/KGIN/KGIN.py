...
class Aggregator(nn.Module):
    ...
    def forward(self, entity_emb, user_emb, latent_emb,
        ...
        return entity_agg, user_agg, score_
...
class GraphConv(nn.Module):
    ...
    def __init__(self, channel, n_hops, n_users,
                 ...
                 ind, n_entities, node_dropout_rate=0.5, mess_dropout_rate=0.1, ):
    ...
    def forward(self, user_emb, entity_emb, latent_emb, edge_index, edge_type,
                interact_mat, kg, item2relations, mess_dropout=True, node_dropout=False, getMultOrder=False):
        ...
        cor = self._cul_cor()
        all_emb = torch.cat([user_emb, entity_emb])
        embLst = [all_emb]
        attention = 0
        for i in range(len(self.convs)):
            entity_emb, user_emb, attention = self.convs[i](entity_emb, user_emb, latent_emb,
                                                 edge_index, edge_type, interact_mat,
        ...
        # item_embs = entity_res_emb[torch.LongTensor(list(kg.keys()))]
        # item_entities = torch.stack(list(kg.values()))
        # args.item_entities = item_entities
        # item_relations = torch.stack(list(item2relations.values()))
        # args.item_relations = item_relations
        # entity_embs = entity_res_emb[torch.LongTensor(item_entities)]
        # relation_embs = self.weight[item_relations]
        # padding_mask = torch.where(item_entities != self.n_entities, torch.ones_like(item_entities),
        ...
        # We = entity_embs
        # e_input = torch.multiply(Wh * We, relation_embs).sum(-1)
        # e = self.leakyrelu(e_input)
        # zero_vec = -9e15 * torch.ones_like(e)
        # attention = torch.where(padding_mask.cuda() > 0, e, zero_vec)
        # attention = F.softmax(attention, dim=1)
        # attention = self.dropout(attention)

        if not getMultOrder:
            return entity_res_emb, user_res_emb, self.weight
        else:
            return embLst
...
class Recommender(nn.Module):
    def __init__(self, data_config, args_config, graph, adj_mat, dataset: Loader):
        ...
        self.emb_size = args_config.latdim
        ...
        self.kg_dict, self.item2relations = self.dataset.get_kg_dict(self.n_items)
    ...
    def forward(self, getMultOrder=False):
        ...
        # entity_gcn_emb, user_gcn_emb, cor, attention =
        return self.gcn(user_emb,
                        ...
                        node_dropout=self.node_dropout, getMultOrder=getMultOrder)
        # u_e = user_gcn_emb[user]
        # pos_e, neg_e = entity_gcn_emb[pos_item], entity_gcn_emb[neg_item]
        # return self.create_bpr_loss(u_e, pos_e, neg_e, cor)

    def generate(self, getMultOrder=False):
        user_emb = self.all_embed[:self.n_users, :]
        item_emb = self.all_embed[self.n_users:, :]
        return self.gcn(user_emb,
                        item_emb,
                        ...
                        self.kg_dict,
                        self.item2relations,
                        mess_dropout=False, node_dropout=False, getMultOrder=getMultOrder)
