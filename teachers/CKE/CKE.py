...
class CKE(nn.Module):
    ...
    def getEmbedding(self, getMultOrder=False):
        if not getMultOrder:
            return self.user_embed.weight, self.entity_embed.weight, self.relation_embed.weight
        else:
            return self.user_embed.weight, self.entity_embed.weight
    ...
    def forward(self, *input, is_train, getMultOrder=False):
        if is_train:
            return self.calc_loss(*input)
        else:
            return self.getEmbedding(getMultOrder)
